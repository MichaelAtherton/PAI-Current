...
**This directory (`PAI/`)** contains all system documentation, tools, user context, and the SKILL.md that defines PAI as a skill. The rest of the system lives alongside it under `${PAI_DIR}/` (hooks, skills, settings, memory).
...
12 hierarchical categories with 49 total skills in `${PAI_DIR}/skills/`, each with a `SKILL.md` defining triggers, workflows, and tools. Skills are the primary capability unit.

### Hooks (`THEHOOKSYSTEM.md`)
21+ event hooks across the session lifecycle: SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, Stop, SessionEnd. Defined in `settings.json`, implemented in `${PAI_DIR}/hooks/`.
...
- **Add a hook:** Create handler in `${PAI_DIR}/hooks/handlers/`, register in `settings.json`
