...
|------|--------|-----------|-----------------|------|
| **Standard** | <2min | 8-16 | 1-2 | Single file, single concern, no design decisions |
| **Extended** | <8min | 16-32 | 3-5 | Most tasks — multiple concerns or files (DEFAULT) |
...
**COMPLEXITY GATE:** Already evaluated in CLAUDE.md before this file was loaded. If you are reading this file, the gate confirmed this task as complex. Proceed directly to FIRST ACTION.
...
- Edit the stub PRD.md (already created at Algorithm entry) to add full content — update frontmatter `effort` field with the determined effort level, and add sections (Context, Criteria, Decisions, Verification) per `${PAI_DIR}/PAI/PRDFORMAT.md`
...
- **WRITE TO PRD (MANDATORY):** For Extended+ effort, add a `### Plan` subsection to `## Context` with technical approach and key decisions.
...
**Full spec:** `${PAI_DIR}/PAI/PRDFORMAT.md` (read during OBSERVE if needed for field details or continuation rules).
