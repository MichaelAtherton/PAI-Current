...
**Location:** `${PAI_DIR}/PAI/Tools/Inference.ts`
...
bun ${PAI_DIR}/PAI/Tools/Inference.ts --level fast "System prompt" "User prompt"

# Standard (Sonnet) - balanced reasoning, typical analysis
bun ${PAI_DIR}/PAI/Tools/Inference.ts --level standard "System prompt" "User prompt"

# Smart (Opus) - deep reasoning, strategic decisions
bun ${PAI_DIR}/PAI/Tools/Inference.ts --level smart "System prompt" "User prompt"

# With JSON output
bun ${PAI_DIR}/PAI/Tools/Inference.ts --json --level fast "Return JSON" "Input"

# Custom timeout
bun ${PAI_DIR}/PAI/Tools/Inference.ts --level standard --timeout 60000 "Prompt" "Input"
```
...
**Location:** `${PAI_DIR}/PAI/Tools/RemoveBg.ts`
...
bun ${PAI_DIR}/PAI/Tools/RemoveBg.ts /path/to/image.png

# Remove background and save to different path
bun ${PAI_DIR}/PAI/Tools/RemoveBg.ts /path/to/input.png /path/to/output.png

# Process multiple images
bun ${PAI_DIR}/PAI/Tools/RemoveBg.ts image1.png image2.png image3.png
```
...
**Location:** `${PAI_DIR}/PAI/Tools/AddBg.ts`
...
bun ${PAI_DIR}/PAI/Tools/AddBg.ts /path/to/transparent.png "#EAE9DF" /path/to/output.png

# Add brand background color
bun ${PAI_DIR}/PAI/Tools/AddBg.ts /path/to/transparent.png --brand /path/to/output.png
```
...
**Location:** `${PAI_DIR}/PAI/Tools/GetTranscript.ts`
...
bun ${PAI_DIR}/PAI/Tools/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID"

# Save transcript to file
bun ${PAI_DIR}/PAI/Tools/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID" --save /path/to/transcript.txt
```
...
**Technical Details:**
- Voice server must be running (`${PAI_DIR}/skills/VoiceServer/`)
...
**Location:** `${PAI_DIR}/PAI/Tools/extract-transcript.py`
...
**Location:** `${PAI_DIR}/PAI/Tools/YouTubeApi.ts`
...
bun ${PAI_DIR}/PAI/Tools/YouTubeApi.ts --channel-stats

# Get video statistics
bun ${PAI_DIR}/PAI/Tools/YouTubeApi.ts --video-stats VIDEO_ID

# Get latest uploads
bun ${PAI_DIR}/PAI/Tools/YouTubeApi.ts --latest-videos
```
...
1. **Add tool file:** Place `.ts` or `.py` file directly in `${PAI_DIR}/PAI/Tools/`
...
2. **Document here:** Add section to this file with:
   - Tool location (e.g., `${PAI_DIR}/PAI/Tools/ToolName.ts`)
   - Usage examples
