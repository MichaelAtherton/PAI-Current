...
**Location:** `${PAI_DIR}/projects/-Users-{username}--claude/`
...
ls -lt ${PAI_DIR}/projects/-Users-{username}--claude/*.jsonl | head -5

# View last session events
tail ${PAI_DIR}/projects/-Users-{username}--claude/$(ls -t ${PAI_DIR}/projects/-Users-{username}--claude/*.jsonl | head -1) | jq .
```
...
bun run ${PAI_DIR}/PAI/Tools/FailureCapture.ts --migrate
```
...
bun run ${PAI_DIR}/PAI/Tools/SessionHarvester.ts --recent 10

# Generate pattern synthesis
bun run ${PAI_DIR}/PAI/Tools/LearningPatternSynthesis.ts --week
```
...
- Moved BACKUPS to `${PAI_DIR}/BACKUPS/` (outside MEMORY)
- Renamed RAW-OUTPUTS to RAW
- All directories now ALL CAPS

**2026-01-05:** v1.0 - Unified Memory System migration
- Previous: `${PAI_DIR}/history/`, `${PAI_DIR}/context/`, `${PAI_DIR}/progress/`
