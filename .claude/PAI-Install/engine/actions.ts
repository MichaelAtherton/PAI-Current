  ...
  if (state.installType === "upgrade") {
    ...
    if (cloneResult !== null) {
      ...
      if (initResult !== null) {
        ...
        await emit({
          event: "message",
          content: "Could not clone PAI repo automatically. You can clone it manually later: git clone https://github.com/danielmiessler/PAI.git ${PAI_DIR}",
        });
      }
    }
  }
...
export async function runConfiguration(
  ...
  // Voice server reads ~/.env, hooks read ${PAI_DIR}/.env
  if (existsSync(envPath)) {
    const symlinkPaths = [
      join(paiDir, ".env"),         // ${PAI_DIR}/.env
...
export async function runVoiceSetup(
  ...
  if (hasElevenLabsKey) {
    ...
    // Ensure symlinks exist at both ${PAI_DIR}/.env and ~/.env
