...
| 5 | **PAI Repository** | Clones the PAI repo to `${PAI_DIR}/` (or updates if already present) |
...
|------|----------|----------|
| `settings.json` | `${PAI_DIR}/settings.json` | Merged config (template + user fields) |
| `.env` | `~/.config/PAI/.env` | `ELEVENLABS_API_KEY=...` |
| `LATEST` | `${PAI_DIR}/PAI/Algorithm/LATEST` | Algorithm version (patched to current) |
| Shell alias | `~/.zshrc` | `alias pai='cd ${PAI_DIR} && claude'` |
...
| Permission denied | Run `chmod -R 755 ${PAI_DIR}` |
...
| Banner shows wrong algorithm version | Check `${PAI_DIR}/PAI/Algorithm/LATEST` contains correct version |
...
- **Single-user** — Installs to `${PAI_DIR}/` for the current user only
