...
Usage:
    python3 CitabilityScorer.py <url>

Returns JSON with citability analysis for all content blocks.
...
def score_passage(text: str, heading: Optional[str] = None) -> dict:
    ...
    year_count = len(re.findall(r"\b20(?:2[3-9]|[3-9]\d)\b", text))
    if year_count > 0:
        sd_score += 2
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 CitabilityScorer.py <url>")
