...
TELOS is {PRINCIPAL.NAME}'s life framework stored in `${PAI_DIR}/PAI/USER/TELOS/`. It contains:
...
!`FILE="$1"; CONTENT="$2"; DESCRIPTION="$3"; bun ${PAI_DIR}/commands/update-telos.ts "$FILE" "$CONTENT" "$DESCRIPTION"`
...
!`head -50 ${PAI_DIR}/PAI/USER/TELOS/updates.md`

## View Specific TELOS File
!`FILE="$1"; cat ${PAI_DIR}/PAI/USER/TELOS/"$FILE"`
...
Example:
```bash
bun ${PAI_DIR}/commands/update-telos.ts "BOOKS.md" "- *Project Hail Mary* by Andy Weir" "Added favorite book: Project Hail Mary"
```
...
The script is at: `${PAI_DIR}/commands/update-telos.ts`

All backups are stored in: `${PAI_DIR}/PAI/USER/TELOS/Backups/`

All changes are logged in: `${PAI_DIR}/PAI/USER/TELOS/updates.md`
