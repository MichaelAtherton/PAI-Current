...
const entityIndex = JSON.parse(
  fs.readFileSync('${PAI_DIR}/skills/parser/entity-index.json', 'utf-8')
);
...
// Main processing
const entityIndexPath = '${PAI_DIR}/skills/parser/entity-index.json';
...
const schema = JSON.parse(
  fs.readFileSync('${PAI_DIR}/skills/parser/schema/content-schema.json', 'utf-8')
);
...
processedArticles.forEach((article: any, index: number) => {
  ...
  if (!valid) {
    ...
    const filename = `${PAI_DIR}/skills/parser/output/${timestamp}_batch-${index + 1}.json`;
...
```bash
# Check entity counts
cat ${PAI_DIR}/skills/parser/entity-index.json | jq '{
...
URLS=("$@")
SKILL_DIR="${PAI_DIR}/skills/parser"
