 ...
 *   User:  ${PAI_DIR}/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
 *
 * Usage:
...
/**
 * Save a composed agent to ${PAI_DIR}/custom-agents/{slug}.md
 *
 * Produces a CLAUDE CODE COMPATIBLE agent file that can be:
 * 1. Copied to ${PAI_DIR}/agents/ and used as a built-in agent
 ...
 *
 * Matches the structural format of built-in agents in ${PAI_DIR}/agents/*.md:
...
\`\`\`bash
bun run ${PAI_DIR}/skills/Agents/Tools/ComposeAgent.ts --load "${slug}"
\`\`\`

Or reconstruct from traits:

\`\`\`bash
bun run ${PAI_DIR}/skills/Agents/Tools/ComposeAgent.ts --traits "${agent.traits.join(",")}"
\`\`\`
...
OPTIONS:
  ...
  -s, --save           Save composed agent to ${PAI_DIR}/custom-agents/
...
CONFIGURATION:
  ...
  User traits:    ${PAI_DIR}/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
  Custom agents:  ${PAI_DIR}/custom-agents/
