...
1. **Load Art preferences:**
   ```
   ${PAI_DIR}/PAI/USER/SKILLCUSTOMIZATIONS/Art/PREFERENCES.md
   ```
...
3. **Use Theme Constants:**
   ```
   ${PAI_DIR}/skills/Remotion/Tools/Theme.ts
   ```

4. **Reference images** (when visual style reference needed):
   ```
   ${PAI_DIR}/skills/Media/Art/Examples/
   ```

## PAI Theme Quick Reference

```typescript
import { PAI_THEME } from '${PAI_DIR}/skills/Remotion/Tools/Theme'
...
```typescript
import { PAI_THEME, titleScreenStyle, fadeInterpolation } from '${PAI_DIR}/skills/Remotion/Tools/Theme'
